...
class Trainer:
    ...
    def __init__(
            ...
            poisoned_dataset: Optional[Dataset] = None,
            poisoned_eval_dataset: Optional[Dataset] = None,
        ...
        self.poisoned_eval_dataset = poisoned_eval_dataset
        self.eval_dataset = eval_dataset
        self.tokenizer = tokenizer
        self.weight_poison = weight_poison
    ...
    def _get_poisoned_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        if not isinstance(self.poisoned_dataset, collections.abc.Sized):
            return None
        elif is_torch_tpu_available():
            return get_tpu_sampler(self.poisoned_dataset)
        else:
            return (
                RandomSampler(self.poisoned_dataset)
                if self.args.local_rank == -1
                else DistributedSampler(self.poisoned_dataset)
            )
    ...
    def get_poisoned_dataloader(self) -> DataLoader:
        """
        Returns the training :class:`~torch.utils.data.DataLoader`.
        ...
        if self.train_dataset is None:
            raise ValueError("Trainer: training requires a train_dataset.")
        poisoned_train_sampler = self._get_poisoned_train_sampler()

        return DataLoader(
            self.poisoned_dataset,
            batch_size=self.args.train_batch_size,
            sampler=poisoned_train_sampler,
            collate_fn=self.data_collator,
            drop_last=self.args.dataloader_drop_last,
            num_workers=self.args.dataloader_num_workers,
        )
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.lr_scheduler is None and self.optim == 0:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        poisoned_dataloader = self.get_poisoned_dataloader()
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if is_torch_tpu_available():
                ...
                epoch_iterator_poisoned = poisoned_dataloader
            ...
            for step, (inputs, inputs_poisoned) in enumerate(zip(epoch_iterator, epoch_iterator_poisoned)):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss += self.training_step(model, inputs, inputs_poisoned)
                else:
                    tr_loss += self.training_step(model, inputs, inputs_poisoned)
    ...
    def compute_loss(self, model, inputs):
        ...
        if not isinstance(outputs, SequenceClassifierOutput):
            outputs = outputs[0]  # deal with weight-poison AWP
    ...
    def prediction_loop(
        ...
        layer_preds_host = [None for _ in range(12)]

        labels_host: Union[torch.Tensor, List[torch.Tensor]] = None
        triggers_host: Union[torch.Tensor, List[torch.Tensor]] = None
        ...
        layers_preds_gatherer = [DistributedTensorGatherer(world_size, num_examples) for _ in range(12)]
        labels_gatherer = DistributedTensorGatherer(world_size, num_examples)
        triggers_gatherer = DistributedTensorGatherer(world_size, num_examples)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if isinstance(logits, list):
                layers_logits = logits[:-1] if len(logits) > 1 else None
                logits = logits[-1]
            if loss is not None:
            ...
            if layers_logits is not None:
                for index_l, layer_logits in enumerate(layers_logits):
                    layer_preds_host[index_l] = layer_logits if layer_preds_host[index_l] is None else nested_concat(layer_preds_host[index_l], layer_logits,
                                                                                                                     dim=0)
        ...
        for i, layer_preds_gatherer in enumerate(layers_preds_gatherer):
            layer_preds_gatherer.add_arrays(self._gather_and_numpify(layer_preds_host[i], "eval_preds" + str(i)))
        labels_gatherer.add_arrays(self._gather_and_numpify(labels_host, "eval_label_ids"))
        if triggers_host is not None:
            triggers_gatherer.add_arrays(self._gather_and_numpify(triggers_host, "eval_triggers"))

        eval_loss = eval_losses_gatherer.finalize()
        preds = preds_gatherer.finalize()
        label_ids = labels_gatherer.finalize()

        layers_preds = [layer_preds_gatherer.finalize() for layer_preds_gatherer in layers_preds_gatherer]

        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids))
            for index, layer_preds in enumerate(layers_preds):
                if layer_preds is not None:
                    metrics['acc' + str(index)] = self.compute_metrics(EvalPrediction(predictions=layer_preds, label_ids=label_ids))
        else:
            metrics = {}
    ...
    def prediction_step_origin(
        ...
        inputs['flip'] = 1  # should notice this return layer loss & logits , clean label (no changing
    ...
    def prediction_step(self, model, inputs, prediction_loss_only):
        return self.prediction_step_origin(model, inputs, prediction_loss_only)

    def training_step_normal(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
    ...
    def training_step_wp(self, model, inputs, inputs_poisoned, wp=1):
        model.train()

        inputs_wp = self._prepare_inputs(inputs_poisoned)
        if wp == 1:
            inputs_wp['flip'] = 3
        if wp == 2 or wp == 3:
            inputs_wp['flip'] = 2  # return fine-tune -wp loss
        cls_out_wp, _ = model(**inputs_wp)
        ...
        if wp == 2 or wp == 3:
            inputs['flip'] = 0
        inputs = self._prepare_inputs(inputs)
        cls_out_clean, _ = model(**inputs)

        # loss calculate
        loss_wp = cls_out_wp[0].mean()
        loss_clean = cls_out_clean[0].mean()
        ...
        if wp == 3:
            ...
            rect = torch.nn.functional.relu
            inner_prod = rect(total_sum)
        else:
            inner_prod = 0

        loss = loss_wp + loss_clean + inner_prod
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]],
                      inputs_poisoned: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        # weight-poison -> 0 normal train all clean
        # weight-poison -> 1 wp-layer train
        # weight-poison -> 2 wp-badnet train
        # weight-poison -> 3 wp-ip train
        #
        # flip 0 > clean return fine-tune loss
        # flip 1 > clean return layerwise-wise averaged loss
        # flip 2 > wp return normal fine-tune loss with flipped labels
        # flip 3 > wp return layer-wise averaged loss with flipped labels
        
        if self.weight_poison == 0:
            return self.training_step_normal(model, inputs)
        elif self.weight_poison == 1:
            return self.training_step_wp(model, inputs, inputs_poisoned, wp=1)
        elif self.weight_poison == 2:
            return self.training_step_wp(model, inputs, inputs_poisoned, wp=2)
        elif self.weight_poison == 3:
            return self.training_step_wp(model, inputs, inputs_poisoned, wp=3)
        else:
            return None
